...
pub(crate) fn cmd_antigravity_config(raw: &[String]) -> Result<i32, String> {
    if raw.is_empty() {
        return Err("Error: antigravity-config requires a subcommand, e.g. 'ensure'".to_string());
    }
    let subcmd = &raw[0];
    if subcmd != "ensure" {
        return Err(format!(
            "Error: unknown antigravity-config subcommand `{subcmd}`. Expected `ensure`."
        ));
    }

    let mut config_path_flag = None;
    let mut mcp_path_flag = None;
    let mut index = 1;
    while index < raw.len() {
        match raw[index].as_str() {
            "--config" => {
                index += 1;
                if index >= raw.len() {
                    return Err("Error: --config requires a value".to_string());
                }
                config_path_flag = Some(raw[index].clone());
            }
            "--mcp" => {
                index += 1;
                if index >= raw.len() {
                    return Err("Error: --mcp requires a value".to_string());
                }
                mcp_path_flag = Some(raw[index].clone());
            }
            other => {
                return Err(format!(
                    "Error: unknown option `{other}` for antigravity-config ensure"
                ));
            }
        }
        index += 1;
    ...
    let mcp_config_path = if let Some(path_str) = config_path_flag {
        std::path::PathBuf::from(path_str)
    } else if let Ok(env_val) = std::env::var("MCP_CONFIG_PATH") {
        std::path::PathBuf::from(env_val)
    } else {
        let home = std::env::var("HOME")
            .map_err(|_| "Error: HOME environment variable is not set".to_string())?;
        std::path::PathBuf::from(home).join(".gemini/config/mcp_config.json")
    };

    let agent_swarm_mcp_path = if let Some(path_str) = mcp_path_flag {
        std::path::PathBuf::from(path_str)
    } else if let Ok(env_val) = std::env::var("AGENT_SWARM_MCP") {
        std::path::PathBuf::from(env_val)
    } else {
        swarm_store::store::swarm_home()
            .ok_or_else(swarm_store::store::swarm_home_err)?
            .join("bin/agent-swarm-mcp")
    };

    let mut config = if mcp_config_path.exists() {
        let content = std::fs::read_to_string(&mcp_config_path).map_err(|err| {
            format!(
    ...
    let mcp_servers_value = config_obj
        .get_mut("mcpServers")
        .ok_or_else(|| "Error: failed to retrieve or create mcpServers".to_string())?;
    let mcp_servers_obj = mcp_servers_value.as_object_mut().ok_or_else(|| {
        format!(
    ...
    mcp_servers_obj.insert(
        ...
        serde_json::json!({
            "command": agent_swarm_mcp_path.to_string_lossy().to_string(),
            "args": [],
            "env": {}
        }),
    );
    ...
    swarm_store::store::write_text_atomic(&mcp_config_path, next_content)?;
...
mod tests {
    ...
    #[test]
    fn test_antigravity_config_missing_creates_expected_structure() {
        let temp_dir = tempfile::tempdir().unwrap();
        let config_file = temp_dir.path().join("mcp_config.json");
        let mcp_bin = "/path/to/test-mcp";
        ...
        let result = cmd_antigravity_config(&args);
        ...
        assert_eq!(
            val["mcpServers"]["agent-swarm"]["command"].as_str(),
            Some(mcp_bin)
        );
        assert_eq!(
            val["mcpServers"]["agent-swarm"]["args"]
                .as_array()
                .unwrap()
                .len(),
            0
        );
        assert!(val["mcpServers"]["agent-swarm"]["env"].is_object());
    }

    #[test]
    fn test_antigravity_config_existing_preserves_others_and_updates_agent_swarm() {
        let temp_dir = tempfile::tempdir().unwrap();
        let config_file = temp_dir.path().join("mcp_config.json");
        ...
        std::fs::write(&config_file, serde_json::to_string(&initial_json).unwrap()).unwrap();

        let mcp_bin = "/path/to/new-mcp";
        let args = vec![
            "ensure".to_string(),
            "--config".to_string(),
            config_file.to_string_lossy().to_string(),
        ...
        assert_eq!(val["otherKey"].as_str(), Some("otherValue"));
        assert_eq!(
            val["mcpServers"]["rust"]["command"].as_str(),
            Some("rust-command")
        );
        assert_eq!(val["mcpServers"]["rust"]["args"][0].as_str(), Some("-v"));
        assert_eq!(
            val["mcpServers"]["rust"]["env"]["DEBUG"].as_str(),
            Some("1")
        );
        ...
        assert_eq!(
            val["mcpServers"]["agent-swarm"]["env"]
                .as_object()
                .unwrap()
                .len(),
            0
    ...
    #[test]
    fn test_antigravity_config_invalid_json_returns_error_and_does_not_overwrite() {
        let temp_dir = tempfile::tempdir().unwrap();
        let config_file = temp_dir.path().join("mcp_config.json");
        ...
        // File remains malformed and not overwritten
        let content = std::fs::read_to_string(&config_file).unwrap();
